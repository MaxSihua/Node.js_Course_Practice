 ...
 *                 $ref: '#/components/schemas/Movie'
 *       404:
 *         description: No movies with this genre were found.
 */
  getMoviesByGenreRouter.get('/:name', async (req: Request, res: Response): Promise<void> => {
    const { name } = req.params;

    if (!name) {
      throw new BadRequestError({code: 400, message: "Name was not provided."});
    }

    const movies = await Movie.find({ genre: {$in: [name] } })
    .then((movies: Object[]) => {

      if ( !movies.length ) {
        throw new BadRequestError({code: 404, message: 'Movie with this genre was not found' });
      }
 ...
 *         description: Title was not provided.
 *       404:
 *         description: No movies with this title were found.
 */
 ...
 *       404:
 *         description: No movies were deleted.
 */
  deleteMovieByIdRouter.delete('/:id', async (req: Request, res: Response): Promise<void> => {
    ...
    const deletedMovie = await Movie.findByIdAndDelete({ _id: new ObjectId(id) })
    .catch((err: Error) => {
        throw new BadRequestError({code: 404, message: err.message });
    });
